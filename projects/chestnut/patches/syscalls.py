...
def get_blocked_syscalls_num(syscalls):
    blocked = []
    with open('csv/syscalls_x86_64.csv') as csvfile:
        tbl = csv.reader(csvfile)
        for row in tbl:
            if int(row[0]) not in syscalls:
                blocked.append(int(row[0]))
    return blocked
...
def modify_elf(syscalls, fname):
    ...
    try:
        ...
        note = lief.ELF.Note.create("IAIK", lief.ELF.Note.TYPE.CORE_ARM_SYSTEM_CALL, flattened_syscalls)
...
def main(fnames):
    ...
    with open(os.path.join(cached_results_folder, "syscalls_allow_%s" % filter_file(fnames[0])), "w") as ff:
        json.dump(sorted(list(all_syscalls)), ff)

    # my modification
    with open(os.path.join(cached_results_folder, "syscalls_blocked_%s" % filter_file(fnames[0])), "w") as ff:
        json.dump(sorted(list(all_blocked)), ff)
