...
def main(fnames):
    ...
    all_blocked_num = syscalls.get_blocked_syscalls_num(used_syscalls)
    ...
    with open(os.path.join(cfg.cached_results_folder, "syscalls_allowed_%s" % filter_file(fnames[0])), "w") as ff:
        json.dump(sorted(list(used_syscalls)), ff)

    # my modification
    with open(os.path.join(cfg.cached_results_folder, "syscalls_blocked_%s" % filter_file(fnames[0])), "w") as ff:
        json.dump(sorted(list(all_blocked_num)), ff)
